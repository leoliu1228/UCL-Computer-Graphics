...
#ifdef SOLUTION_HALTON
// Put your code here
// Code based on Halton Sequence: https://en.wikipedia.org/wiki/Halton_sequence
int i = sampleIndex;
int b = prime(dimensionIndex);
float f = 1.0;
float r = 0.0;

// Since while statement is not usable here, we simply utilise the for loop and manually terminate the loop when i<0
for (int j=0;j<100;j++){
f = f/float(b);
r = r + f * float(mod(i,b));
i = i/b;
...
#ifdef SOLUTION_HALTON
// Put your code here
// As advised in the coursework requirements, using halton based random yelds a better and more efficient result
// However, only using halton(baseSampleIndex,dimensionIndex) will give a structural patterns during the path tracing process.
// The halton does give a better pattern but as we can see the parameters of the function, it takes dimension and sample and it will return
// the same value for all the pixel in the path when passing identical parameters
// To solve this issue, we can simply give it a random offset e.g. uniformRandom()
// However using uniformRandom does solve the pattern issue, the result it produces is actually not good due to the way it works e.g. some pixels not getting enough hits.
// And the uniformRandom() is not a true RNG, similar to C++ programming where we actually need a seed to randomize it every time we run the function
// Thus using pixelSeed() actually gives a better result and faster.
// Since the value should be between 0 and 1, the fract() has been used to get the decimal part.
...
#ifdef SOLUTION_BOUNCE
// Put your code here
// The function/formula is provided in the coursework pdf, simply plug in all the variables and compute the direction
// Can also use simple(i), simple(i+1) for xi.x and xi.y separately
vec2 xi = sample2(dimensionIndex);
// As formula
float theta = acos(2.0 * xi[0] - 1.0);
...
// Put your code here
// According to the Phong BRDF formula
// fr(wi,wo) = ks * (( n + 2)/( 2 * Pi)) * (wo ⋅ r)+ ^ n
// ks = Specular
// n = Glossiness
// wi = In direction
// wo = Out direction
...
return ks * (( n + 2.0)/( 2.0 * M_PI)) * pow(max(0.0,dot(wo,r)),n);
#else
return vec3(1.0);
#endif
}
...
#ifdef SOLUTION_THROUGHPUT
// Put your code here
// This compute the cos(theta) in svBRDF, which is calculated from out direction and normal
// Since the BRDF should not be negative, any value less than 0 should be ignored.
// The geometry term should not be negative value.
return vec3(max(0.0,dot(normalize(outDirection), normal)));
#else
return vec3(1.0);
#endif
}
...
#ifdef SOLUTION_BOUNCE
// Put your code here
// Almost same as what we did in raytracing
outgoingRay.origin = hitInfo.position;
// As advised in the coursework pdf
outgoingRay.direction = normalize(randomDirection(PATH_SAMPLE_DIMENSION+2*i));
#else
...
#ifdef SOLUTION_THROUGHPUT
// Put your code here
// The throughput = BRDF * geometric term where the BRDF part can be combined with different functions e.g. Lambertian diffuse + glossy refelction
// In the above tasks we only compute the refelction term and if we use it only, it will render a scene where those sphere have shadows, reflections but have no color
// Therefore we need to add the diffuse term to the BRDF part
// After testing different diffuse functions, we figured out that using the unaltered diffuse value actually gives the best visual result
vec3 diffuseTerm = hitInfo.material.diffuse;
vec3 specularTerm = getReflectance(hitInfo.material, hitInfo.normal, incomingRay.direction, outgoingRay.direction);
vec3 geometricTerm = getGeometricTerm(hitInfo.material, hitInfo.normal, incomingRay.direction, outgoingRay.direction);
throughput *= (diffuseTerm + specularTerm) * geometricTerm;
#else
...
#ifdef SOLUTION_AA
// Put your code here
// The AA can be done using a simple 1 x 1 box filter, the random offsets are given using uniformRandom() within the range from -0.5 to 0.5 so that it will be inside the 1 x 1 box
// Since this function is also called during the iteration, in each iteraction, it will trace the path from a random position to the target pixel, and it will then automatcially average the color inside the pixel after each loop
// And fianlly the sharp edge will be blurry since the AA takes effect
// This can achieve the exact same AA quality ("pin point accuracy", even the shapes after AA are identical) as the reference image in the coursework pdf when using 256x128 resolution
vec2 sampleCoord = fragCoord + vec2(uniformRandom()-0.5,uniformRandom()-0.5);

#else
// Change this to get anti-aliasing
...
#ifdef SOLUTION_LIGHT
// Put your code here
scene.spheres[0].material.emission = vec3(0.9,0.5,0.3);
#endif
...
#ifdef SOLUTION_LIGHT
// Put your code here
scene.spheres[1].material.emission = vec3(0.3,0.9,0.8);
#endif
